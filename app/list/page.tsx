    ...
    <main className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-blue-50">
      <div className="container mx-auto max-w-4xl px-4 py-5">
        <div className="mb-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setShowAddForm(true)}
              className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-pink-500 to-rose-500 px-4 py-2 text-xs text-white transition-all duration-300 hover:scale-105 hover:shadow-lg">
        ...
        <div className="mb-6 text-center">
          <div className="mb-2 inline-flex h-14 w-14 items-center justify-center rounded-full bg-pink-100">
            <Heart className="h-7 w-7 fill-pink-600 text-pink-600" />
          </div>
          <h1 className="mb-1.5 text-2xl font-bold text-gray-800 md:text-3xl">Love List</h1>
          <p className="text-sm text-gray-600">恋爱列表 - 你我之间的约定</p>
        </div>

        <div className="mb-5 grid grid-cols-3 gap-2.5">
          <div className="rounded-2xl bg-white p-2.5 text-center shadow-md">
            <p className="text-xl font-bold text-gray-800">{todos.length}</p>
            <p className="text-xs text-gray-500">总计</p>
          </div>
          <div className="rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 p-2.5 text-center shadow-md">
            <p className="text-xl font-bold text-green-600">{todos.filter(t => t.completed).length}</p>
            <p className="text-xs text-gray-600">已完成</p>
          </div>
          <div className="rounded-2xl bg-gradient-to-br from-pink-50 to-rose-50 p-2.5 text-center shadow-md">
            <p className="text-xl font-bold text-pink-600">{todos.filter(t => !t.completed).length}</p>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {todos.map((todo) => (
              <div key={todo.id} className={`rounded-2xl bg-white p-3 shadow-md transition-all duration-300 hover:shadow-lg ${todo.completed ? 'opacity-60' : ''}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className={`text-sm ${todo.completed ? 'line-through text-gray-400' : 'text-gray-800'}`}>{todo.title}</p>
        ...
        {showAddForm && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowAddForm(false)}>
            <div className="w-full max-w-lg rounded-3xl bg-white p-5" onClick={(e) => e.stopPropagation()}>
              <h2 className="mb-4 text-lg font-bold text-gray-800">添加新约定</h2>
              ...
              <div className="mt-5 flex gap-3">
                ...
                <button onClick={handleAdd} className="flex-1 rounded-xl bg-gradient-to-r from-pink-500 to-rose-500 px-4 py-2 text-xs text-white transition-all hover:shadow-lg">添加</button>
              </div>
            </div>
          </div>
        )}
